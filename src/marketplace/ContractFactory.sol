...
contract ContractFactory is Ownable, IContractFactory {
    ...
    /// @notice FIX 3: каждая лицензия используется только один раз
    ...
    function registerImplementation(
        ...
        _versions[contractId].push(ImplVersion({
            addr:       implementation,
            metadata:   metadata,
            deprecated: false
        }));
    ...
    function deployContractWithVersion(
        ...
        if (version >= vers.length || vers[version].addr == address(0))
            revert ImplementationNotFound();
        if (vers[version].deprecated) revert ImplementationNotFound();
    ...
    function deployContractBatch(
        ...
        if (contractIds.length != licenseTokenIds.length) revert ArrayLengthMismatch();
    ...
    {
        ImplVersion[] storage vers = _versions[contractId];
        if (version >= vers.length) revert ImplementationNotFound();
    ...
    function _checkLicense(bytes32 contractId, uint256 tokenId) internal {
        // Проверяем владельца
        (bool ok, bytes memory data) = address(licenseNFT).staticcall(
            abi.encodeWithSignature("ownerOf(uint256)", tokenId)
        );
        if (!ok) revert LicenseNotValid(tokenId);
        address tokenOwner = abi.decode(data, (address));
        if (tokenOwner != msg.sender) revert NotLicenseOwner(tokenId);

        // Проверяем валидность
        if (!licenseNFT.isLicenseValid(tokenId)) revert LicenseNotValid(tokenId);

        // Проверяем совпадение contractId
        bytes32 licContractId = licenseNFT.tokenContractId(tokenId);
        if (licContractId != contractId) revert LicenseMismatch(contractId, licContractId);

        // FIX 3: проверяем что лицензия ещё не использована
        if (_licenseUsed[tokenId]) revert LicenseAlreadyUsed(tokenId);
        _licenseUsed[tokenId] = true;
    }

    function _activeVersion(bytes32 contractId) internal view returns (uint256) {
        ImplVersion[] storage vers = _versions[contractId];
        if (vers.length == 0) revert ImplementationNotFound();
        for (uint256 i = vers.length; i > 0; --i) {
            if (!vers[i - 1].deprecated) return i - 1;
        }
        revert ImplementationNotFound();
    }

    function _deploy(bytes32 contractId, uint256 version, uint256 licenseTokenId)
        internal returns (address clone)
    {
        ImplVersion storage v = _versions[contractId][version];
        clone = v.addr.clone();

        // FIX 2: корректная обработка initialize
        (bool ok,) = clone.call(
            abi.encodeWithSignature("initialize(address)", msg.sender)
        );
        if (!ok) {
            // Если initialize существует но упал — revert
            // Проверяем наличие селектора в bytecode через staticcall
            (bool hasInit,) = clone.staticcall(
                abi.encodeWithSignature("initialize(address)", msg.sender)
            );
            // staticcall тоже упадёт если функция меняет стейт,
            // но если вернул false — значит функция есть и упала
            if (!hasInit) revert InitializationFailed();
            // если hasInit = true — функция не существует, это нормально
        }
        ...
        // licenseTokenId используется только для проверки — не храним
